...
export const Route = createFileRoute("/booking-tickets/unapproved")({
  head: () => ({
    meta: [
      { title: "Unapproved Tickets | Helloji Booking Desk" },
      { name: "description", content: "Review and approve pending booking tickets" },
    ],
  }),
  component: UnapprovedTicketsPage,
});
...
function UnapprovedTicketsPage() {
  ...
  const [approveTicket, setApproveTicket] = useState<BookingTicket | null>(null);
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      q.set("status", "unapproved");
      if (search.trim()) q.set("search", search.trim());

      const res = await authFetch(`/api/booking-tickets?${q.toString()}`);
      if (!res.ok) {
        throw new Error("Failed to load unapproved tickets");
      }
  ...
  // Approval handling
  const openApprove = (t: BookingTicket) => {
    setApproveTicket(t);
    setApprovalRemark(t.remark === "N/A" ? "" : t.remark || "");
  };
  ...
  const handleConfirmApproval = async () => {
    ...
    setApproving(true);
    try {
      const res = await authFetch(`/api/booking-tickets/${approveTicket._id}/approve`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ remark: approvalRemark.trim() || "Approved" }),
      });
      if (!res.ok) {
        const d = await res.json();
        throw new Error(d.message || "Failed to approve ticket");
      }
      toast.success(`Ticket ${approveTicket.btId} approved successfully!`);
  ...
  return (
    <DashboardShell
      ...
      subtitle="Pending booking tickets awaiting approval"
      actions={
    ...
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>&gt;</span>
          <span className="text-foreground font-medium">Unapproved Tickets</span>
        </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={12} className="p-8 text-center text-muted-foreground">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span>Loading unapproved tickets...</span>
                      </div>
                    </td>
                  </tr>
                ) : error ? (
                  ...
                  tickets.map((t, idx) => (
                    <tr key={t._id} className="hover:bg-muted/30 transition-colors">
                      ...
                      {/* Action buttons: Edit, View, Print, Approve (with Remark), Delete */}
                      <td className="px-3 py-3 text-center whitespace-nowrap sticky right-0 bg-card/95">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <Button
                            ...
                            title="Approve Ticket (Change to Approved with Remark)"
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm py-2">
              ...
              <div>
                ...
                <p className="text-amber-600 font-medium">{viewTicket.approvedBy || "Unapproved"}</p>
              </div>
              <div>
      ...
      {/* Approval Modal with Remark Field */}
      {approveTicket && (
        <Dialog open={!!approveTicket} onOpenChange={(open) => !open && setApproveTicket(null)}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-green-700 dark:text-green-400">
                <CheckCircle2 className="size-5" />
                Approve Booking Ticket {approveTicket.btId}
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-4 py-2 text-sm">
              <p className="text-muted-foreground">
                You are about to change this ticket from <span className="font-semibold text-foreground">Unapproved</span> to{" "}
                <span className="font-semibold text-green-600">Approved</span>. Please provide an approval remark.
              </p>
              <div className="p-3 bg-muted/60 rounded-md space-y-1 text-xs">
                <p><span className="font-semibold">Guest:</span> {approveTicket.guestName}</p>
                <p><span className="font-semibold">Product:</span> {approveTicket.product}</p>
                <p><span className="font-semibold">Sale By:</span> {approveTicket.saleBy || "—"}</p>
              </div>
              <div className="space-y-2">
                <Label htmlFor="approval-remark" className="text-sm font-medium">
                  Approval Remark <span className="text-xs text-muted-foreground">(e.g. Ok Jk, Confirmed with supplier)</span>
                </Label>
                <Input
                  id="approval-remark"
      ...
      <DeleteConfirm
        ...
        description="Are you sure you want to move this unapproved booking ticket to the Recycle Bin? You can restore it later."
